				...
				<div class="col-md-12">
					<div class="card">									
						<div class="card-body">
							<div class="panel panel-primary">
								<div class="tab_wrapper page-tab">
									<ul class="tab_list">
										...
										<li class="active">
											<a href="{!! url('/workingtype/list/edit/'.$editid )!!}">
												<span class="visible-xs"></span>
							...
							<div class="row">
								<div class="col-md-12 col-sm-12 col-xs-12">
									<form action="update/{{ $working->id }}" method="post"  enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left">
										<div class="row">
											<div class="col-12 col-md-6">
												<label class="form-label" for="first-name">{{ trans('app.Workingtype')}} <label class="text-danger">*</label>
												</label>
												<input type="text"  required="required" name="working" value="{{ $working->name }}"  class="form-control">
											</div>
											<div class="col-12 col-md-6">
															...
															<select id="vehicle_category" name="type" class="form-control vehicle_type">
																@if(!empty($types))
																	@foreach($types as $type)
																		<option
																		@if($working->type_id == $type->id)
																			selected
																		@endif
																		value="{{ $type->id }}">{{ $type->vehicle_type }}</option>
																	@endforeach
																@endif
															</select>
														</div>
