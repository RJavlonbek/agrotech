							...
							<div class="table-responsive">							
								...
								<?php $i=1;?>
								 @foreach($workingtypes as $workingtype)
									<tr>
										<td>{{ $i }}</td>
										<td>{{ $workingtype->name }}</td>
										<td> 
											<a href="{!! url ('/workingtype/list/edit/'.$workingtype->id) !!}"> <button type="button" class="btn btn-round btn-success">{{ trans('app.Edit')}}</button></a>
									   
											<a url="{!! url('/workingtype/list/delete/'.$workingtype->id)!!}" class="sa-warning"> <button type="button" class="btn btn-round btn-danger dgr">{{ trans('app.Delete')}}</button></a>
										</td>
									</tr>
								<?php $i++; ?>
								@endforeach
