...
@if (CheckAccessUser('vehicle_num', $userid, 'create')=='yes')
							...
							<div class="row">
								...
								<div class="col-md-6 form-group">
									...
									<select class="form-control select-type custom-select" name="type" required="required">
										<option value="">Davlat raqami turini tanlang</option>
											...
											<select class="form-control select-state custom-select" name="state_id">
												...
												@if(!empty($states))
													...
													@foreach($states as $state)
														<option value="{{$state->id}}" state-code="{{$state->code}}">
								...
								<div class="col-md-2 form-group">
									...
									<input class="form-control" type="text" name="series" pattern="[A-Z]{2}" onkeyup="this.value=this.value.toUpperCase()" required="required" title="2 ta harf kiriting!" placeholder="AA" maxlength="2" />
								...
								<div class="col-md-4 form-group">
									...
									<input class="form-control" type="text" name="number" pattern="[0-9]{3}" required="required" title="3 ta raqam kiriting!" placeholder="777" maxlength="3" />
								...
								<div class="col-md-3 recover-field">
									<div class="form-group">
										...
										<select class="form-control reason" name="recover_reason">
											<option value=""></option>
											<option value="lost_a">Yo'qolgan eski namunadagi DRBga yangi DRB berish</option>
											<option value="lost_b">Yo'qolgan yangi namunadagi DRBga yangi DRB berish</option>
								...
								<div class="col-md-3">
									...
									<button class="btn btn-success float-right" type="submit" disabled="disabled">Saqlash</button>
								</div>
							</div>
						</form>
					</div>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('select.select-state, input[name="series"], input[name="number"], select.select-type').change(function(){
			checkIfNumberExist();
		});
		...
		function checkIfNumberExist(){
			...
			var type = $('select.select-type').val();

			var submitButton=$('#transport-number-form button[type="submit"]');
			if(code && series && number && type){
				...
				$.ajax({
					...
					success:function(data){
						...
						if(data=='exist'){
							...
							submitButton.attr('disabled','disabled');
							swal('Kiritilgan davlat raqami mavjud!','Viloyat kodini, seriya, raqam yoki DRB turini o\'zgartirishingiz mumkin','error');
							return 'exist';
						}else{
