				...
				<div class="col-md-12">
					...
					<div class="card">									
						...
						<div class="card-body">
							...
							<form action="{{ url('/report/ownership') }}" method="get">
								<input type="hidden" name="form" value="{{$form}}">
								<div class="row">
									<div class="col-md-6">
										@if(!empty($states))
										<label class="form-label">Viloyat tanlang</label>
										@elseif(!empty($cities))
										<label class="form-label">Tuman / shahar tanlang</label>
										@endif
							...
							<div class="card-body">
								<div class="row mb-2">
									<div class="report-header fs-16 col-8">
										@if(!empty($state))
											<span><b>{{$state->name}}</b>dagi mavjud qishloq xo'jaligi, meliorativ va yo'l qurilish texnikalarining rusumlari to'g'risida <b>{{date('d.m.Y')}}</b> holatiga ma'lumot</span>
										@elseif(!empty($city))
											<span><b>{{$city->name}}</b>dagi mavjud qishloq xo'jaligi, meliorativ va yo'l qurilish texnikalarining rusumlari to'g'risida <b>{{date('d.m.Y')}}</b> holatiga ma'lumot</span>
										@else
											<span>Mavjud texnika - hisobot</span>
										@endif
									</div>
									<div class="col-4 text-right">
										<div class="btn btn-primary" id="export-excel" filename="{{$state?$state->name:$city->name}}-mulkchiligi {{date('d.m.Y')}}">Excel ga jo'natish</div>
										<div class="btn btn-primary print-table-button" table="report" >Chop etish</div>
									</div>
								</div>
								<div class="sticky-table sticky-headers sticky-ltr-cells">
									<table class="table table-bordered" id="report">
										<thead class="sticky-header" style="position: relative; position: sticky;">
											...
											<tr class="sticky-header">
												...
												<th class="sticky-custom">Nomi</th>

												<th class="sticky-custom">Modeli</th>
