...
@if (CheckAccessUser('report_old', $userid, 'read')=='yes')
    	...
    	<div class="section">	
	        <div class="row">
				<div class="col-md-12">
					<div class="card">
						<div class="card-body">
							<form action="{{ url('/report/vehicle-age') }}" method="get">
							...
							<div class="card-body">
								<div class="row mb-2">
									<div class="report-header fs-16 col-8">
										@if(!empty($state))
											<span><b>{{$state->name}}</b>da mavjud asosiy turdagi qishloq xo'jaligi, meliorativ va yo'l qurilish texnikalarning foydalanib kelinayotgan davrlari to'g'risida <b>{{date('d.m.Y')}}</b> holatiga ma'lumot</span>
										@elseif(!empty($city))
											<span><b>{{$city->name}}</b>da mavjud asosiy turdagi qishloq xo'jaligi, meliorativ va yo'l qurilish texnikalarning foydalanib kelinayotgan davrlari to'g'risida <b>{{date('d.m.Y')}}</b> holatiga ma'lumot</span>
										@else
								...
								<div class="sticky-table sticky-headers sticky-ltr-cells">
									<table class="table table-bordered" id="report">
										<thead>
											<tr class="sticky-header sticky-row sticky-height">
												<th class="sticky-cell" rowspan="4">№</th>
												<th class="height" colspan="2">Texnika</th>
												<th rowspan="4">
													<span class="">Jami texnika soni, dona</span>
												</th>
												<th colspan="10">
													<span class="font-weight-bold">Foydalanish muddatlari har besh yillik bo'yicha</span>
												</th>
											</tr>
											<tr class="sticky-header sticky-height1">
												<th class="sticky-custom" rowspan="3">Nomi</th>
												<th class="sticky-custom" rowspan="3">Modeli</th>
												<th class="sticky-custom" colspan="4">
													1-5 yil
												</th>
											...
											<tr class="sticky-header sticky-height2">
												...
												<th class="sticky-custom2" colspan="2">
													shundan 2019 yilda olingan
												</th>
												<th class="sticky-custom2" rowspan="2">
													soni, dona
												</th>
...
$(document).ready(function() {
	...
	var height1 = parseFloat($('.sticky-height').height())-1;
	var height2 = height1 + parseFloat($('.sticky-height1').height());
	var height3 = height2 + parseFloat($('.sticky-height2').height())	;
	$('.sticky-custom').css('top',height1 + 'px');
	$('.sticky-custom2').css('top',height2 + 'px');
	$('.sticky-custom3').css('top',height3 + 'px');
	$('.sticky-row th').css('top', '-1px');
	console.log($('.height'));
