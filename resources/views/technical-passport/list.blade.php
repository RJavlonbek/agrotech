							...
							<div class="card-body">
								...
								<div id="list-date-filter">
									...
									<div class="float-right-buttons">
										...
										<div class="export-passport-button btn btn-primary  mr-2 float-right-button" table='examples1' filename='Texnik pasportlar'><i class="fa fa-file-excel-o"></i> Excelga jo'natish</div>
									</div>
								</div>
								<div class="row">
									<div class="col-8"></div>
											...
											@if(!empty($passports))
												@foreach($passports as $passport)
													<tr>
														...
														<td>
															@if (CheckAccessUser('vehicle_pass', $userid, 'read')=='yes')
																<a href="/vehicle/technical-passport/preview?id={{$passport->id}}&details=true">{{ $passport->series.$passport->number }}</a>
															@else
														...
														@if (CheckAdmin($userid)=='yes')
															<td class="no-print"> 
																<a day="{{$passport->day}}" class="btn btn-round btn-success cancel" url="{!! url ('/vehicle/technical-passport/cancel/'.$passport->id) !!}"  href="">{{ trans('app.Cancel')}}</a>
													    	</td>
														@endif
													</tr>
													<?php $i++; ?>
...
<script>
	$(document).ready(function(){
		...
		$(".export-passport-button").on('click', function(){
			window.location.href = '{{URL::to('/export/technical-passport')}}';
		});
